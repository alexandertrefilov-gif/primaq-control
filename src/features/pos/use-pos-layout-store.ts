...
// einstellbar, aber beide quadratisch (width = height = <field>Px).
// clamp() lässt sie auf schmaleren Viewports schrumpfen, ohne den
// gewählten Wert je zu überschreiten und ohne das 1:1-Verhältnis zu
// verlieren — Referenzbreite 1366px (Standard-iPad-Querformat).
export function cardSizeClamp(px: number): string {
  const floor = Math.max(100, px - 40);
  const preferredVw = ((px / 1366) * 100).toFixed(2);
  return `clamp(${floor}px, ${preferredVw}vw, ${px}px)`;
}
...
export type LayoutConfig = {
  ...
  productCardSizePx: number; // 120–240 px, step 10, default 180 — Sortenkarten (quadratisch)
  sizeCardSizePx: number;    // 120–240 px, step 10, default 180 — Größenkarten (quadratisch)
